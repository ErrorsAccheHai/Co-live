...
router.post('/', auth, roleCheck('landlord'), upload.array('images', 5), async (req, res) => {
    try {
        ...
        const propertyData = {
            ...
            pricing: (() => {
                const p = parseMaybeJSON(req.body.pricing);
                if (p && typeof p === 'object') return p;
                // fallback: use price as monthly rate
                const monthly = Number(req.body.price || req.body.rent) || 0;
                return { dailyRate: 0, monthlyRate: monthly, yearlyRate: 0 };
            })(),
...
router.put('/admin/verify/:id', auth, roleCheck('admin'), async (req, res) => {
    try {
        ...
        if (status === 'approved') {
            ...
            property.keepLive = true; // make it visible on public listings
        }
